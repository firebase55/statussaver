...
import com.AQif.Statusdanwloder.PreviewActivity;
...
public class WAppStatusAdapter extends RecyclerView.Adapter<WAppStatusAdapter.ViewHolder> {
    ...
    public WAppStatusAdapter(Context activity, ArrayList<DataModel> jData) {
        this.mData = jData;
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        if (!this.file.isDirectory()) {
            if (!Utils.getBack(jpast.getFilePath(), "((\\.mp4|\\.webm|\\.ogg|\\.mpK|\\.avi|\\.mkv|\\.flv|\\.mpg|\\.wmv|\\.vob|\\.ogv|\\.mov|\\.qt|\\.rm|\\.rmvb\\.|\\.asf|\\.m4p|\\.m4v|\\.mp2|\\.mpeg|\\.mpe|\\.mpv|\\.m2v|\\.3gp|\\.f4p|\\.f4a|\\.f4b|\\.f4v)$)").isEmpty()) {
                try {
                    Glide.with(this.activity).load(this.file).apply(new RequestOptions().placeholder(android.R.color.black).error(android.R.color.black).optionalTransform(new RoundedCorners(5))).into(holder.imagevi);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            ...
            holder.downloadIV.setOnClickListener(v -> savemedia(jpast.getFilePath(),activity));
        }
    }
